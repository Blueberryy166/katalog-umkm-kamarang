...
<div class="max-w-4xl mx-auto space-y-6">
    <div class="flex items-center justify-between">
        <div>
            <nav class="flex items-center gap-2 text-xs text-slate-400 mb-1">
                ...
                <span class="text-slate-700 font-bold">Edit Produk</span>
            </nav>
            <h2 class="text-xl font-extrabold text-slate-900">Edit Produk: {{ $product->name }}</h2>
        </div>
    ...
    <form action="{{ route('admin.products.update', $product->id) }}" method="POST" enctype="multipart/form-data" class="bg-white rounded-3xl border border-slate-200 shadow-xs p-6 sm:p-8 space-y-6">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <select name="umkm_id" required class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-xs focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:bg-white transition-all">
                    @foreach($umkms as $u)
                        <option value="{{ $u->id }}" {{ old('umkm_id', $product->umkm_id) == $u->id ? 'selected' : '' }}>{{ $u->name }} ({{ $u->owner_name }})</option>
                    @endforeach
                </select>
            </div>
            ...
            <div class="sm:col-span-2">
                ...
                <input type="text" name="name" value="{{ old('name', $product->name) }}" required class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-xs focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:bg-white transition-all">
            </div>

            <!-- Harga Minimum / Dasar -->
            <div>
                <label class="block text-xs font-bold text-slate-700 mb-1.5">Harga Dasar / Minimum (Rp) <span class="text-rose-500">*</span></label>
                <input type="number" name="price" value="{{ old('price', (int)$product->price) }}" required min="0" step="100" placeholder="Contoh: 10000" class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-xs focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:bg-white transition-all">
            ...
            <div>
                ...
                <input type="number" name="price_max" value="{{ old('price_max', $product->price_max ? (int)$product->price_max : '') }}" min="0" step="100" placeholder="Contoh: 20000" class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-xs focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:bg-white transition-all">
            ...
            <div>
                ...
                <input type="text" name="unit" value="{{ old('unit', $product->unit) }}" required placeholder="pouch 250gr, toples 300gr, pack, kg, pcs" class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-xs focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:bg-white transition-all">
            </div>
            ...
            <div>
                ...
                <select name="stock_status" required class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-xs focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:bg-white transition-all">
                    <option value="available" {{ old('stock_status', $product->stock_status) == 'available' ? 'selected' : '' }}>Tersedia (Ready Stock)</option>
                    <option value="preorder" {{ old('stock_status', $product->stock_status) == 'preorder' ? 'selected' : '' }}>Pre-Order</option>
                    <option value="out_of_stock" {{ old('stock_status', $product->stock_status) == 'out_of_stock' ? 'selected' : '' }}>Stok Habis</option>
                </select>
            </div>

            <!-- Varian Produk (Rasa, Ukuran, Kemasan) -->
            <div class="sm:col-span-2 bg-slate-50 p-5 rounded-2xl border border-slate-200" x-data="{ 
                variants: {{ json_encode(old('variants', $product->variants ?? [])) ?: '[]' }}, 
                newVariant: '',
            ...
            <div class="sm:col-span-2">
                ...
                <input type="text" name="pirt_number" value="{{ old('pirt_number', $product->pirt_number) }}" placeholder="Contoh: P-IRT: 2153209010123-26" class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-xs focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:bg-white transition-all">
            </div>

            <!-- Foto Utama -->
            <div class="sm:col-span-2">
                <label class="block text-xs font-bold text-slate-700 mb-1.5">Ganti Foto Produk (Kosongkan jika tidak diubah)</label>
            ...
            <div class="sm:col-span-2">
                ...
                <textarea name="short_description" rows="2" class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-xs focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:bg-white transition-all">{{ old('short_description', $product->short_description) }}</textarea>
            </div>

            <!-- Deskripsi Lengkap -->
            <div class="sm:col-span-2">
                <label class="block text-xs font-bold text-slate-700 mb-1.5">Deskripsi Lengkap / Keunggulan / Komposisi</label>
                <textarea name="full_description" rows="5" class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-xs focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:bg-white transition-all">{{ old('full_description', $product->full_description) }}</textarea>
            </div>
