...
@section('title', 'Edit Data UMKM: ' . $umkm->name)
...
<div class="max-w-4xl mx-auto space-y-6">
    <div class="flex items-center justify-between">
        <div>
            <nav class="flex items-center gap-2 text-xs text-slate-400 mb-1">
                ...
                <span class="text-slate-700 font-bold">Edit UMKM</span>
            </nav>
            <h2 class="text-xl font-extrabold text-slate-900">Edit Data: {{ $umkm->name }}</h2>
        </div>
    ...
    <form action="{{ route('admin.umkms.update', $umkm->id) }}" method="POST" enctype="multipart/form-data" class="bg-white rounded-3xl border border-slate-200 shadow-xs p-6 sm:p-8 space-y-6">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <input type="text" name="name" value="{{ old('name', $umkm->name) }}" required class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-xs focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:bg-white transition-all">
            </div>

            <!-- Nama Pemilik -->
            <div>
                <label class="block text-xs font-bold text-slate-700 mb-1.5">Nama Pemilik Usaha <span class="text-rose-500">*</span></label>
                <input type="text" name="owner_name" value="{{ old('owner_name', $umkm->owner_name) }}" required class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-xs focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:bg-white transition-all">
            </div>
            ...
            <div>
                ...
                <input type="text" name="phone" value="{{ old('phone', $umkm->phone) }}" placeholder="Contoh: 082119876543 (Kosongkan jika tidak ada)" class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-xs focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:bg-white transition-all">
            ...
            <div>
                ...
                <input type="text" name="dusun" value="{{ old('dusun', $umkm->dusun) }}" placeholder="Contoh: Dusun Manis, Dusun Pahing, Dusun Kliwon, Dusun Wage" class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-xs focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:bg-white transition-all">
            </div>

            <!-- Alamat Lengkap -->
            <div class="sm:col-span-2">
                <label class="block text-xs font-bold text-slate-700 mb-1.5">Alamat Lengkap Usaha</label>
                <input type="text" name="address" value="{{ old('address', $umkm->address) }}" class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-xs focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:bg-white transition-all">
            </div>
            ...
            <div class="sm:col-span-2 bg-slate-50 p-5 rounded-2xl border border-slate-200 space-y-4">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="text" id="latitude" name="latitude" value="{{ old('latitude', $umkm->latitude ?? '-6.84667744') }}" placeholder="-6.84667744" class="w-full px-3.5 py-2 bg-white border border-slate-200 rounded-xl text-xs font-mono focus:outline-none focus:ring-2 focus:ring-emerald-500 transition-all">
                    </div>
                    <div>
                        <label class="block text-[11px] font-bold text-slate-700 mb-1">Longitude (Garis Bujur)</label>
                        <input type="text" id="longitude" name="longitude" value="{{ old('longitude', $umkm->longitude ?? '108.54561525') }}" placeholder="108.54561525" class="w-full px-3.5 py-2 bg-white border border-slate-200 rounded-xl text-xs font-mono focus:outline-none focus:ring-2 focus:ring-emerald-500 transition-all">
                    </div>
                </div>
                ...
                <div>
                    ...
                    <input type="text" id="maps_url" name="maps_url" value="{{ old('maps_url', $umkm->maps_url) }}" placeholder="https://maps.google.com/?q=..." class="w-full px-3.5 py-2 bg-white border border-slate-200 rounded-xl text-xs focus:outline-none focus:ring-2 focus:ring-emerald-500 transition-all">
            ...
            <!-- Logo / Foto Toko -->
            <div class="sm:col-span-2">
                <label class="block text-xs font-bold text-slate-700 mb-1.5">Ganti Foto Toko / Logo Usaha (Kosongkan jika tidak diubah)</label>
                <div class="flex items-center gap-4 mb-2">
                    ...
                    <span class="text-xs text-slate-500">Logo/foto toko saat ini</span>
                </div>
            ...
            <div class="sm:col-span-2">
                ...
                <textarea name="description" rows="4" class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-xs focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:bg-white transition-all">{{ old('description', $umkm->description) }}</textarea>
            </div>
            ...
            <div class="sm:col-span-2 bg-slate-50 p-4 rounded-2xl flex items-center justify-between border border-slate-200">
                ...
                <label class="relative inline-flex items-center cursor-pointer">
                    <input type="checkbox" name="is_active" value="1" {{ old('is_active', $umkm->is_active) ? 'checked' : '' }} class="sr-only peer">
